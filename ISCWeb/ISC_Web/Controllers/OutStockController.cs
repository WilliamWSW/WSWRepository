...
namespace ISC_Web.Controllers
{
    ...
    {
        // GET: OutStock
        public ActionResult OutstockHis(string id, int? page, string txtirsterminalid_input, string txtirsGoodsCode,string txtirsGoodsName, string txtirsdatetimepicker1, string txtirsdatetimepicker2)
        {
            //id是客户端屏幕分辨率
            ...
            List<ViewModels.VM_OutStockHis> result = new List<ViewModels.VM_OutStockHis>();
            DateTime start;
            DateTime end;
            string vterminalid = "";
            //判断查询开始时间是否为空
            ...
            result = Service.OutStock.OutstockHis.GetOutStockDtHis(vterminalid, txtirsGoodsCode, txtirsGoodsName, start, end);
            ...
            //以上两种方法各有好处,推荐使用Bootstrap,响应式布局支持各种大小屏幕,并且不会影响用户体验.JStable和CSStable在小屏幕上全部数据重叠在一起,无法识别.
